...
import {
  NC_TABLE_OPERATORS,
  NC_TABLE_BOOLEAN_OPTIONS,
  NC_TABLE_YES_NO_OPTIONS,
} from "../NcTableConfig";
...
const defaultOperators: SearchOperator[] = NC_TABLE_OPERATORS.map((op) => ({
  value: op.value,
  label: op.text,
}));
...
const NcTableSearch = <T,>({
  ...
  const [searchConditions, setSearchConditions] = useState<SearchCondition[]>([
    { id: 1, column: "all", operator: "~=", value: "" },
  ]);
  ...
  const addSearchCondition = () => {
    setSearchConditions([
      ...searchConditions,
      { id: nextConditionId, column: "all", operator: "~=", value: "" },
    ]);
              ...
              <div className="flex w-full gap-2">
                {(() => {
                  const selectedColumn = columns.find(
                    (col) => col.key === condition.column
                  );
                  const dataType =
                    selectedColumn?.searchOverride?.dataType || "text";
                  ...
                  if (dataType === "select") {
                    const options =
                      selectedColumn?.searchOverride?.selectOptions || [];
                    return (
                      <Select
                        value={condition.value}
                        onValueChange={(value) =>
                          updateSearchCondition(condition.id, "value", value)
                        }
                      >
                  ...
                  if (dataType === "boolean") {
                    return (
                      ...
                      >
                        ...
                        <SelectContent>
                          {NC_TABLE_BOOLEAN_OPTIONS.map((opt, idx) => (
                            <SelectItem
                              key={`bool-${idx}`}
                              value={String(opt.value)}
                            >
                  ...
                  if (dataType === "YesOrNo") {
                    return (
                      ...
                      >
                        ...
                        <SelectContent>
                          {NC_TABLE_YES_NO_OPTIONS.map((opt, idx) => (
                            <SelectItem
                              key={`yn-${idx}`}
                              value={String(opt.value)}
                            >
